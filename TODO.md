...
# Connected Clips - behavior when trimming the parent clip (Implemented)
...
- ~~if during right-trim, the handle moves past the beginning of the connected clip, a gap clip is inserted to anchor the connected clip~~
- ~~the gap clip is only permanently added when the user releases the mouse while it is visible~~
